...
class Centering_transactions extends Model
{
    use HasFactory, SoftDeletes;

    protected $table = 'centering_transactions';
    ...
    {
        $fields = [
            'centering_transactions.id',
            'centering_transactions.uuid',
            'centering_transactions.id AS encryptid',
            'centering_transactions.quantity',
             DB::raw('CASE WHEN centering_transactions.type = 1 THEN "TRANSFER" WHEN centering_transactions.type = 2 THEN "RECEIVED" ELSE "ISSUED" END AS type,
                DATE_FORMAT(centering_transactions.created_at, "%d-%b-%Y") AS date_created'),
        ];
    ...
    public static function storeRecords(Request $request)
    {
        ...
        if(count($getdata) > 0){
            Materials_stock::where(['materials_category_id'=>2,'site_id'=>$site_id])
