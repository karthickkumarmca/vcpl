...
class Cement_transactions extends Model
{
    ...
    protected $table = 'cement_transactions';
    ...
    {
        $fields = [
            'cement_transactions.id',
            'cement_transactions.uuid',
            'cement_transactions.id AS encryptid',
            'cement_transactions.quantity',
             DB::raw('CASE WHEN cement_transactions.type = 1 THEN "TRANSFER" WHEN cement_transactions.type = 2 THEN "RECEIVED" ELSE "ISSUED" END AS type,
                DATE_FORMAT(cement_transactions.created_at, "%d-%b-%Y") AS date_created'),
        ];
    ...
    public static function storeRecords(Request $request)
    {
        ...
        $data->type                        = $request->selected_tab;
        if($request->selected_tab==2){
