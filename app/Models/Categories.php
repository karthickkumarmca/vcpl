...
class Categories extends Model
{
    ...
    public static function getcategories($page, $offset, $sort, $search_filter)
    {
        $fields = [
            'categories.id',
            'categories.uuid',
            'categories.id AS encryptid',
            'categories.category_name',
            'categories.status as status_id',
            DB::raw('CASE WHEN categories.status = 1 THEN "Active" ELSE "In-Active" END AS status, DATE_FORMAT(categories.created_at, "%d-%b-%Y %r") AS date_created'),
        ];
