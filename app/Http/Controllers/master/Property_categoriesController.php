...
class Property_categoriesController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        if (!config("roles.{$role}.property_categories_management")) {
            abort(403);
        } else {
            if ($request->has('request_type')) {
                ...
                $role = session('user_role');
                $create_access     = config("roles.{$role}.property_categories_management_access.create");
                $view_access     = config("roles.{$role}.property_categories_management_access.view");
                $edit_access     = config("roles.{$role}.property_categories_management_access.edit");
                $delete_access   = config("roles.{$role}.property_categories_management_access.delete");
                $change_status_access   = config("roles.{$role}.property_categories_management_access.change_status");
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.property_categories_management_access.create")) {
            abort(403);
        } else {
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.property_categories_management_access.view")) {
            abort(403);
        } else {
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.property_categories_management_access.edit")) {
            abort(403);
        } else {
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.property_categories_management_access.delete")) {
            abort(403);
        } else {
