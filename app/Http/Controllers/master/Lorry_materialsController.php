...
class Lorry_materialsController extends Controller
{
    /**
    ...
    public function list(Request $request)
    {
        ...
        if (!config("roles.{$role}.lorry_materials_management")) {
            abort(403);
        } else {
            if ($request->has('request_type')) {
                ...
                if (!empty($records['records'])) {
                    ...
                    $statusCode = '400';
                    $message    = "No lorry_materials found";
                    $data       = $records;
                }
                ...
                $role = session('user_role');
                $create_access     = config("roles.{$role}.lorry_materials_management_access.create");
                $view_access     = config("roles.{$role}.lorry_materials_management_access.view");
                $edit_access     = config("roles.{$role}.lorry_materials_management_access.edit");
                $delete_access   = config("roles.{$role}.lorry_materials_management_access.delete");
                $change_status_access   = config("roles.{$role}.lorry_materials_management_access.change_status");
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.lorry_materials_management_access.create")) {
            abort(403);
        } else {
            ...
            return view('master.lorry_materials.create',compact('categories','units'));
        }
    }
    public function store(Request $request)
    {
        ...
        if (!config("roles.{$role}.lorry_materials_management")) {
            ...
            $request['material_id']= 3;
            if($request->has('lorry_materials_id')){
            ...
            return redirect('master/lorry-materials/list'); 
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.lorry_materials_management_access.view")) {
            abort(403);
        } else {
            ...
            if ($centering_materials) {
                ...
                return view('master.lorry_materials.view', $data);
            } else {
                $data = [
                    'message' => "Invalid lorry_materials"
                ];
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.lorry_materials_management_access.edit")) {
            abort(403);
        } else {
            ...
            if ($centering_materials) {
                ...
                return view('master.lorry_materials.edit', $data);
            } else {
                $data = [
                    'message' => "Invalid lorry_materials"
                ];
    ...
    public function updateStatus($id)
    {
        ...
        if (!config("roles.{$role}.lorry_materials_management_access.edit")) {
            ...
            $data = [
                'redirect_url' => url(route('lorry-materials-list'))
            ];
    ...
    {
        $role = session('user_role');
        if (!config("roles.{$role}.lorry_materials_management_access.delete")) {
            abort(403);
        } else {
