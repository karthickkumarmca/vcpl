...
<section class="content-header">
    <h1 class="col-lg-6 no-padding">Clients <small>Management</small></h1>
    <ol class="breadcrumb">
                ...
                <div class="box-header with-border">
                    <div class="box-tools">
                        <div class="btn-group">
                            @isset($create_access)
                            @if ($create_access == 1)
                            <a href="{{ url(route('create-clients')) }}" class="btn btn-sm btn-primary"><i class="fa fa-plus fa-fw"></i>Create Clients</a>
                            @endif
                            @endisset
                        </div>
                    </div>
...
<script type="text/javascript">
            ...
            'action_button' : {
                ...
                'action': [
                {
                    ...
                    },
                    {
                        ...
                        "title"     : 'Change status',
                        "url"       : function(data){
                            return "clients-update-status/"+data['uuid'];
                        },
                        "icon"      : "fa fa-check",
                        "method"    : "get",
                        "condition" : function(data){
                        ...
                        "function_call" : "changeclientsStatus"
                    },
                    {
                        "name"      : "In-Active",
                        "type"      : "dialog", //view,dialog,modal
                    ...
                    },
                    {
                        ...
                        "title"     : 'Delete',
                        "url"       : function(data){
                            return "clients/delete/"+data['uuid'];
                        },
                        "icon"      : "fa fa-trash",
                        "method"    : "get",
                        /*"condition" : function(data){
         ...
         * @param Object current object
         */
         function changeclientsStatus(current) {
            var is_confirm = $(current).data('is_confirm');
            var method = $(current).data('method');
            var url = $(current).data('url');
            if(is_confirm) {
                ...
                swal({
                    ...
                    if (isConfirm) {
                        updateClientsStatus(url, method);
                    }
                });
            }else {
                updateClientsStatus(url, method);
         ...
         * @param string method
         */
         function updateClientsStatus(url, method) {
            var request  = {
                'url'             : url,
                'type'            : method,
                'data'            : {},
